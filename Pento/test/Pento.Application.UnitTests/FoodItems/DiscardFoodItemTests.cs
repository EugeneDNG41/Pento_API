...
internal sealed class DiscardFoodItemTests
{
    ...
    private readonly DiscardFoodItemCommandHandler _handler;

    public DiscardFoodItemTests()
    {
        ...
        _handler = new DiscardFoodItemCommandHandler(
            _converter,
            _userContext,
            _foodItemRepo,
            _hub,
    ...
    private static DiscardFoodItemCommand CreateCommand(Guid foodItemId, decimal qty, Guid unitId)
        => new(foodItemId, qty, unitId);
    ...
    // ---------- TEST 5 ----------
    [Test]
    public async Task Handle_ShouldDiscard_WhenSameUnit()
    {
        var householdId = Guid.NewGuid();
        var unitId = Guid.NewGuid();
    ...
    // ---------- TEST 6 ----------
    [Test]
    public async Task Handle_ShouldDiscard_WhenConvertedUnit()
    {
        var householdId = Guid.NewGuid();
        var itemUnit = Guid.NewGuid();
        var requestUnit = Guid.NewGuid();
    ...
    // ---------- TEST 7 ----------
    [Test]
    public async Task Handle_ShouldNotifyHouseholdAfterDiscard()
    {
        var householdId = Guid.NewGuid();
        var unitId = Guid.NewGuid();
        FoodItem foodItem = CreateFoodItem(householdId, unitId, 5);
