    ...
    IUnitOfWork unitOfWork) : ICommandHandler<CreateCompartmentCommand, Guid>
{
    public async Task<Result<Guid>> Handle(CreateCompartmentCommand command, CancellationToken cancellationToken)
    {
        ...
        if (await compartmentRepository.AnyAsync(c => c.Name.Equals(command.Name, StringComparison.OrdinalIgnoreCase) && c.HouseholdId == householdId && c.StorageId == command.StorageId, cancellationToken))
        {
