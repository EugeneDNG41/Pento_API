...
internal sealed class GetMyTradeReportsQueryHandler(
    ISqlConnectionFactory factory,
    IUserContext userContext)
    : IQueryHandler<GetMyTradeReportsQuery, IReadOnlyList<TradeReportResponse>>
{
    public async Task<Result<IReadOnlyList<TradeReportResponse>>> Handle(
        GetMyTradeReportsQuery query,
        CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            ...
            WHERE tr.reporter_user_id = @UserId
              AND tr.is_deleted = FALSE
            ORDER BY tr.created_on DESC;
        """;

        IEnumerable<TradeReportResponse> reports =
            await connection.QueryAsync<TradeReportResponse>(
                sql,
                new { UserId = userId });

        return Result.Success<IReadOnlyList<TradeReportResponse>>(reports.ToList());
    }
}
