...
internal sealed class CreateFoodItemBulkCommandHandler(
    ...
    IUnitOfWork unitOfWork) : ICommandHandler<CreateFoodItemBulkCommand>
{
    public async Task<Result> Handle(CreateFoodItemBulkCommand bulk, CancellationToken cancellationToken)
    {
        Guid? householdId = userContext.HouseholdId;
        foreach (CreateFoodItemCommand command in bulk.Commands)
        {
            ...
            if (compartment.HouseholdId != householdId)
            {
